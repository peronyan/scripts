...
def checkSmali_Multi(apk,targetSmalis,codeSnippet):
	global countNum
	for smali in targetSmalis:
		#for codeSnippet in codeSnippets:
			#print "smali:"+ smali
			#print "codeSnippet:" + codeSnippet
			#getData =[]
		flag,className,score = checkJava2SmaliCode(smali,codeSnippet)
		if flag:
			countNum += 1
			writeOutput(apk,smali,className,codeSnippet,score,str(countNum))

def checkJava2SmaliCode(smali,codeSnippet):
	invokeList = []
	className = ""
	flag = True
	for cSmali in open(smali):
		if flag:
			className = cSmali
			flag = False
		#print cSmali
		if cSmali.find("invoke") > -1:
			invokeList.append(cSmali)

	score = calcScore(invokeList,codeSnippet)
	if score >= SCORE_THRESHOLD:
		#print className,str(score)
		return True,className,score
	else:
		return False,className,score
		
def calcScore(invokeList,codeSnippet):
	codes = []
	for snippet in open(codeSnippet):
		snippet = snippet.strip()
		codes.append(snippet)
	score = 0.0
	doneFlag = []
	addScore = float(100.0/len(codes))
	firstFlag = True
	firstIndex = -1
	for index_c,code in enumerate(codes):
		for index_m,invokeMethod in enumerate(invokeList):
			if invokeMethod.find(code) > -1:
				if firstFlag:
					firstIndex = index_m
					firstFlag = False
				#print invokeMethod
				if firstIndex > -1:#print code
					denominator = (float)(math.fabs((index_m +1) - firstIndex))
					#print denominator
					if denominator < 1:
						denominator = 1.0
					score += (addScore/denominator)
					firstIndex += 1
					break
	return score
				#invokeMethod.pop(index_m)
	

def doApktool(apkPath):
	print "apktool:" + apkPath
	if not os.path.isdir(TEMP_DIR):
		cmd = 'mkdir %s'%(TEMP_DIR)
		subprocess.call(shlex.split(cmd))
	workdir = TEMP_DIR + apkPath.split("/")[-1].rstrip(".apk")

	cmd = 'apktool d -q -f %s -o %s'%(apkPath,workdir)
	subprocess.call(shlex.split(cmd))
...
def writeOutput(apk,smali,className,codeSnippet,score,countNum):
	appName = apk.replace(APK_DIR,"")
	global outfName
	...
	smali = smali.lstrip("./")
	#codeSnippet = codeSnippet.replace(CHECK_SMALI_DIR,'')
	codeSnippet = codeSnippet.strip()
	className = className.strip()
	text = appName +","+smali+","+className+","+codeSnippet+","+str(score)+","+countNum+os.linesep
...
def doDecompile(apk,checksmali,index):
	global countNum
	countNum = 0
	...
	fName = root.split("/")[-1]
	fName = TEMP_DIR + fName.rstrip(".apk") 
	doApktool(apk)
	smaliFName = str("./" + fName + "/smali/")
	smaliPathLists = getAppSmaliPath(smaliFName)
	checkSmali_Multi(apk,smaliPathLists,checksmali)
...
if __name__ == "__main__":
	...
	for index,apk in enumerate(apkLists):
		func_args.append((doDecompile,apk,sys.argv[1],index))
